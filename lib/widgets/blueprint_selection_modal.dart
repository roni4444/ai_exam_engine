...
class BlueprintSelectionModal extends StatefulWidget {
  final Function(String fileName) onSelect;
  const BlueprintSelectionModal({super.key, required this.onSelect});

  @override
  State<BlueprintSelectionModal> createState() => _BlueprintSelectionModalState();
}

class _BlueprintSelectionModalState extends State<BlueprintSelectionModal> {
  List<dynamic> _files = [];
  bool _loading = true;
  ...
  Future<void> _loadFiles() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        _showError('Failed to load blueprints: $e');
      }
    }
  }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('Choose blueprints from the database', style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
              ],
            ),
          ),
          IconButton(
        ...
        Expanded(
          child: GridView.builder(
            ...
            itemBuilder: (context, index) {
              final ExamBlueprint file = _files[index];
